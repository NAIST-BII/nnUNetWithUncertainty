...
def maybe_load_checkpoint(nnunet_trainer: nnUNetTrainer, continue_training: bool, validation_only: bool,
    ...
    if continue_training:
        print('Entered continue training')
        expected_checkpoint_file = join(nnunet_trainer.output_folder, 'checkpoint_final.pth')
        if not isfile(expected_checkpoint_file):
            expected_checkpoint_file = join(nnunet_trainer.output_folder, 'checkpoint_latest.pth')
            print('Checkpoint was set as ', expected_checkpoint_file)
        ...
        expected_checkpoint_file = None
    print('Checkpoint status ', expected_checkpoint_file)
    if expected_checkpoint_file is not None:
        nnunet_trainer.load_checkpoint(expected_checkpoint_file)
        print(f'Checkpoint {expected_checkpoint_file} loaded!')
...
def run_training(dataset_name_or_id: Union[str, int],
    ...
    if num_gpus > 1:
        ...
        # print(f"Trainer setting time: \t{time.time() - st:0.3f} sec")
        if disable_checkpointing:
            nnunet_trainer.disable_checkpointing = disable_checkpointing
        ...
        print(f"Load check point time: \t{time.time() - st:0.3f} sec")

        st = time.time()
        if torch.cuda.is_available():
            cudnn.deterministic = False
            cudnn.benchmark = True
        # print(f"Cuda setting time: \t{time.time() - st:0.3f} sec")

        st = time.time()
        if not only_run_validation:
            nnunet_trainer.run_training()
        # print(f"One epoch training time: \t{time.time() - st:0.3f} sec")
