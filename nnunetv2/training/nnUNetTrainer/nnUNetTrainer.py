...
class nnUNetTrainer(object):
    ...
    def get_tr_and_val_datasets(self):
        ...
        dataset_tr = nnUNetDataset(self.preprocessed_dataset_folder, tr_keys,
                                   folder_with_segs_from_previous_stage=self.folder_with_segs_from_previous_stage,
                                   num_images_properties_loading_threshold=0)
        # print(f"Getting training dataset time: \t\t{time() - st:0.3f} sec")
        st = time()
        dataset_val = nnUNetDataset(self.preprocessed_dataset_folder, val_keys,
                                    folder_with_segs_from_previous_stage=self.folder_with_segs_from_previous_stage,
                                    num_images_properties_loading_threshold=0)
        # print(f"Getting validation dataset time: \t\t{time() - st:0.3f} sec")
    ...
    def get_dataloaders(self):
        ...
        st = time()
        deep_supervision_scales = self._get_deep_supervision_scales()
        # print(f"Deep supervision time: \t\t{time() - st:0.3f} sec")
        
        st = time()
        rotation_for_DA, do_dummy_2d_data_aug, initial_patch_size, mirror_axes = \
            self.configure_rotation_dummyDA_mirroring_and_inital_patch_size()
        # print(f"Dummy rotation time: \t\t{time() - st:0.3f} sec")
        ...
        # print(f"Transformations time: \t\t{time() - st:0.3f} sec")
        # validation pipeline
        ...
        print(f"Plain dataloaders time: \t\t{time() - st:0.3f} sec")
        st = time()
        allowed_num_processes = get_allowed_n_proc_DA()
        print(f"Allowed n proc DA {allowed_num_processes} time: \t\t{time() - st:0.3f} sec")
        st = time()
        if allowed_num_processes == 0:
            ...
            mt_gen_val = LimitedLenWrapper(self.num_val_iterations_per_epoch, data_loader=dl_val,
                                           ...
                                           num_cached=max(6, allowed_num_processes // 4), 
                                           seeds=None, pin_memory=self.device.type == 'cuda',
                                           wait_time=0.02)
        # print(f"LimitedLenWrapper time: \t\t{time() - st:0.3f} sec")
        return mt_gen_train, mt_gen_val

    def get_plain_dataloaders(self, initial_patch_size: Tuple[int, ...], dim: int):
        st = time()
        dataset_tr, dataset_val = self.get_tr_and_val_datasets()
        print(f"Getting training/validation datasets time: \t\t{time() - st:0.3f} sec")
    ...
    def on_train_end(self):
        ...
        print(f"Saving checkpoint time: \t\t{time() - st:0.3f} sec")
        ...
        # print(f"Shutting down dataloader time: \t\t{time() - st:0.3f} sec")

        st = time()
        empty_cache(self.device)
        self.print_to_log_file("Training done.")
        print(f"Emptying cash time: \t\t{time() - st:0.3f} sec")
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        print(f"Batch calling time: \t\t{time() - st:0.3f} sec")
        ...
        print(f"Sending target to device time: \t\t{time() - st:0.3f} sec")
        ...
        print(f"Auto cast time: \t\t{time() - st:0.3f} sec")
        ...
        if self.grad_scaler is not None:
            ...
            print(f"Grad scaler time: \t\t{time() - st:0.3f} sec")
        else:
            st = time()
            l.backward()
            torch.nn.utils.clip_grad_norm_(self.network.parameters(), 12)
            self.optimizer.step()
            print(f"Optimizer time: \t\t{time() - st:0.3f} sec")
        return {'loss': l.detach().cpu().numpy()}

    def on_train_epoch_end(self, train_outputs: List[dict]):
        st = time()
        outputs = collate_outputs(train_outputs)
        # print(f"Collate outputs time: \t\t{time() - st:0.3f} sec")
        st = time()
        if self.is_ddp:
        ...
        print(f"Stacking loss time: \t\t{time() - st:0.3f} sec")
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        data = batch['data']
        target = batch['target']
        # print(f"Validation data reading: \t\t{time() - st:0.3f} sec")
        ...
        if isinstance(target, list):
            ...
            target = target.to(self.device, non_blocking=True)
        # print(f"Validation sending to devices: \t\t{time() - st:0.3f} sec")
        ...
        output = output[0]
        target = target[0]
        # print(f"Validation highest resolution: \t\t{time() - st:0.3f} sec")
        ...
        if self.label_manager.has_ignore_label:
            ...
            mask = None
        # print(f"Validation post processing: \t\t{time() - st:0.3f} sec")
        ...
        if not self.label_manager.has_regions:
            ...
            fp_hard = fp_hard[1:]
            fn_hard = fn_hard[1:]
        # print(f"Validation one hot and cpu finshing time: \t\t{time() - st:0.3f} sec")
    ...
    def on_validation_epoch_end(self, val_outputs: List[dict]):
        ...
        print(f"Validation ending time: \t\t{time() - st:0.3f} sec")
    ...
    def run_training(self):
        st = time()
        self.on_train_start()
        print(f"Trainer starting time: \t\t{time() - st:0.3f} sec")

        for epoch in range(self.current_epoch, self.num_epochs):
            st = time()
            self.on_epoch_start()
            print(f"Epoch starting time: \t\t{time() - st:0.3f} sec")
            
            st = time()
            self.on_train_epoch_start()
            print(f"Train starting time: \t\t{time() - st:0.3f} sec")
            ...
            for batch_id in range(self.num_iterations_per_epoch):
                ...
                st = time()
                tmp_sample = next(self.dataloader_train)
                print(f"Getting the sample time: \t\t{time() - st:0.3f} sec")
                st = time()
                tmp_out = self.train_step(tmp_sample)
                print(f"Running the training step time: \t\t{time() - st:0.3f} sec")
                st = time()
                train_outputs.append(tmp_out)
                # print(f"Output appending time: \t\t{time() - st:0.3f} sec")
            print(f"Total training time: \t\t{time() - st_train:0.3f} sec")
            st = time()
            self.on_train_epoch_end(train_outputs)
            print(f"Train ending time: \t\t{time() - st:0.3f} sec")

            with torch.no_grad():
                st = time()
                self.on_validation_epoch_start()
                print(f"Validation starting time: \t\t{time() - st:0.3f} sec")
            ...
            st = time()
            self.on_epoch_end()
            print(f"Epoch ending time: \t\t{time() - st:0.3f} sec")
