...
def resample_data_or_seg_to_shape(data: Union[torch.Tensor, np.ndarray],
    ...
    """
    # print('Converting to numpy if needed', flush=True)
    ...
    if data is not None:
        assert len(data.shape) == 4, "data must be c x y z"
    # print('Going to the resampling...', flush=True)
    data_reshaped = resample_data_or_seg(data, new_shape, is_seg, axis, order, do_separate_z, order_z=order_z)
    return data_reshaped
...
def resample_data_or_seg(data: np.ndarray, new_shape: Union[Tuple[float, ...], List[float], np.ndarray],
    ...
    # print('Getting resize function...', flush=True)
    if is_seg:
    ...
    if np.any(shape != new_shape): #np.any(shape != new_shape):# False
        # print('Entered resampling', flush=True)
        ...
        if do_separate_z:
            ...
            for c in range(data.shape[0]):
                ...
                if shape[axis] != new_shape[axis]:
                    # print('If shape is not equal ', flush=True)
            ...
            # print('Stacking vertically ', flush=True)
            reshaped_final_data = np.vstack(reshaped_final_data)
        else:
            # print("no separate z, order", order)
            reshaped = []
            # print(data.shape[0], flush=True)
            for ii, c in enumerate(range(data.shape[0])):
                # print(f'Reshaping array {ii}', flush=True)
                reshaped.append(resize_fn(data[c], new_shape, order, **kwargs)[None])
            # time.sleep(3)
            # print('Stacking vertically ', flush=True)
            reshaped_final_data = np.vstack(reshaped)
        # time.sleep(3)
        # print('Returning reshaped final data ', flush=True)
        return reshaped_final_data.astype(dtype_data)
    else:
        # print("no resampling necessary")
        # print('No resampling is done ', flush=True)
        return data
